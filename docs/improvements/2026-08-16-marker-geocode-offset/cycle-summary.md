...
## 후속(운영자 필수 작업)
1. **`0039` 적용**(Supabase SQL Editor). backfill 채택 좌표를 살리기 위한 **선행 전제 조건** — 미적용 시 채택 좌표가 다음 sync에 덮인다.
2. **dryRun 관측**: `GET /api/internal/backfill-geocode?dryRun=1&limit=500`(`Authorization: Bearer ${CRON_SECRET}`) → `offset.meanNorthM/meanEastM/meanM/medianM`으로 계통 오차 방향·크기 확정, `adoptRate`/`rejected`로 가드 작동 확인.
3. **실행**: 수치 납득 시 dryRun 없이 `?limit=500`을 `cursor.wrapped=true` 까지 반복(약 22회, ~11,000곳).
4. **회귀 확인**: 다음 `sync-opinet` 응답의 `coordPreserved`와 표본 좌표 불변 확인.
