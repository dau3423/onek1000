...
## 후속 과제 (최종 리뷰가 "머지 후"로 분류한 항목)

머지를 막지 않는다고 판정했지만 사라지면 안 되는 것들이다. SDD 작업공간(`.superpowers/`)은 정리되므로 여기 남긴다.

1. **degrade 감지가 1차 오류를 전부 삼킨다** — `lib/api/place-reviews.ts`
   마이그레이션 미적용 판정이 `if (primary.error)` 라 무관한 오류(예: 23505)에도 gas 는 레거시 형태로 재시도하고, ev/carwash 는 사유가 틀린 503 `migration_required` 를 반환한다.
   실패를 성공으로 둔갑시키지는 않는다(재시도 행은 실제로 기록되고, 읽기가 coalesce 라 레거시 행도 정상 표시).
   ⚠️ 감지를 `42703` 으로 좁히는 "명백한 수정"은 오히려 위험하다 — 오류 시그니처가 다를 때 미적용 환경에서 500 이 되살아난다. 고친다면 시그니처를 실측한 뒤에 좁혀야 한다.

2. **부분 실패 시 모더레이션 카드 표시 지연** — `app/admin/reviews/ReviewsAdminClient.tsx`
   `dismissCard` 가 부분 실패 시 `load()` 를 건너뛰어, 이미 처리된 신고가 계속 대기중으로 보인다. 오류 메시지는 정직하고("M/N 실패") 기각이 멱등이라 재시도로 자가치유된다. 수정은 catch/finally 에 `load()` 한 줄.

3. **테스트 하니스 기법의 결함(재발 방지)** — 스크래치패드 자산
   `mock-loader*.mjs` 는 **절대 지정자**로 매칭된 모듈만 스텁한다. 스텁 대상 모듈을 **상대 경로로 재import** 하는 파일은 스텁을 우회해 실제 코드에 도달하고, 빈 결과가 "통과"로 읽힌다. 이후 같은 기법으로 검증할 때는 `STUB_ABS` 방식(경로 정규화 후 매칭)을 쓸 것.

4. **최종 리뷰 Minor 10건** — 머지 후 정리 대상. 실제 목록은 리뷰 산출물이 정리되며 사라졌으므로, 필요하면 `git log 89f06ad..` 범위를 다시 리뷰하는 편이 빠르다.
