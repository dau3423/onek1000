...
| 항목 | 결정 |
|---|---|
| 총괄 방식 | `pm` 서브에이전트가 한 번 호출되면 사이클 전체를 내부에서 순차 실행 |
...
- 도구: Read, Bash, Glob, Grep, Write, ToolSearch + 브라우저 MCP 도구
...
5. **구현**: senior-developer (기준: plan.md + design.md)
6. **리뷰**: code-reviewer. ❌/⚠️이면 지적 사항을 senior-developer에 전달해 수정 → 재리뷰.
   **최대 2회 루프**, 그래도 ❌면 실패 종료.
7. **QA**: qa-tester. 실패 시 senior-developer 수정 후 **재검증 1회**. 재실패면 실패 종료.
8. **마무리(통과 시)**: PM이 한국어 컨벤션으로 커밋(**push 금지 — push는 사용자가 직접**),
