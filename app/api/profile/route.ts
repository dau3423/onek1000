...
//        또는 { carwashNotifyOptIn: boolean }로 세차 지수 알림 수신 토글.
...
export async function GET(req: Request) {
  ...
  let carwashNotifyOptIn = false;
  const full = await sb
    .from('users')
    .select('nickname, image_url, alimtalk_opt_in, phone, forecast_notify_opt_in, carwash_notify_opt_in')
    .eq('id', userId)
    .maybeSingle();
  if (full.error?.code === '42703') {
    // alimtalk_opt_in(0017)/phone(0018)/forecast_notify_opt_in(0027)/carwash_notify_opt_in(0053)
    // 중 일부 미적용 환경.
    ...
    carwashNotifyOptIn = Boolean(full.data?.carwash_notify_opt_in);
  }
  return NextResponse.json({ nickname, image, alimtalkOptIn, phone, forecastNotifyOptIn, carwashNotifyOptIn });
}
...
export async function PATCH(req: Request) {
  ...
  // 세차 지수 알림 수신 토글(본인 스코프). 발송은 carwash-notify 배치가 담당.
  if (typeof body.carwashNotifyOptIn === 'boolean') {
    const userId = await getUserId(session.user.email);
    if (!userId) return NextResponse.json({ error: 'user not found' }, { status: 404 });
    const sb = getSupabase();
    const { error } = await sb
      .from('users')
      .update({ carwash_notify_opt_in: body.carwashNotifyOptIn })
      .eq('id', userId);
    if (error) {
      // 0053 미적용 환경: 컬럼 없음(42703) → 마이그레이션 안내
      if (error.code === '42703') {
        return NextResponse.json({ error: '알림 설정 준비 중이에요. 잠시 후 다시 시도해 주세요.' }, { status: 503 });
      }
      return NextResponse.json({ error: '변경에 실패했어요.' }, { status: 500 });
    }
    return NextResponse.json({ carwashNotifyOptIn: body.carwashNotifyOptIn });
  }
