...
const RULES: { brand: RepairBrand; keywords: string[] }[] = [
  ...
  // ── 자동차검사소 규칙은 **의도적으로 없다** ──
  // 예전에는 여기서 ['검사소','자동차검사','종합검사','검사정비'] 로 이름을 보고 판별했다.
  // 실측 결과 34,172곳 중 121곳만 잡혔다 — '○○모터스' 같은 지정정비사업자는 간판에 '검사'가
  // 없어 이름만으로는 원천적으로 판별이 불가능했다.
  // 지금은 검사소 표준데이터(lib/inspection/*, 0050)를 직접 적재하고, 지도 RPC 가
  // brand='inspection' 으로 합쳐 내보낸다. 여기서 다시 추론하면 같은 업체가 두 번 그려진다.
];
