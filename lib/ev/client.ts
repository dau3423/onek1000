...
/**
 * 시도 zcode 16개 (data.go.kr EvCharger 기준, 2026-08-22 zcode 01~99 전수 스캔으로 확인).
 *
 * ⚠️ 행정구역 개편 반영: 광주광역시(29) + 전라남도(46) → **전남광주통합특별시(12)**.
 *    29·46 은 이제 totalCount=0 만 준다(3/3 재현). 12 를 넣지 않으면 그 지역 충전소가 통째로 누락된다.
 *    (강원 42→51, 전북 45→52 특별자치도 전환은 이미 반영돼 있었다.)
 *    같은 목록이 services/ev-sync/sync.js 에도 있다 — 바꿀 때 양쪽 다 고칠 것.
 */
